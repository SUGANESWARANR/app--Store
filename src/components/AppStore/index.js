...
class AppStore extends Component {
  state = {usActive: tabsList.tabId}

  changeOn = value => {
    this.setState({usActive: value})
  }

  changedPage = () => {
    const {usActive} = this.state
    const filtered = appsList.filter(each => each.category === usActive)
  ...
  render() {
    const {usActive} = this.state
    const {filtered} = this.changedPage()
    ...
    return (
      <div>
        ...
        <ul>
          {tabsList.map(each => (
            <TabItem
              ...
              isActive={tabsList.tabId === usActive}
            />
          ))}
        </ul>
        ...
        <ul>
          {filtered.map(each => (
            <AppItem filteredDetails={each} key={each.appId} />
          ))}
        </ul>
      </div>
    )
